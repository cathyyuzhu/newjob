...
**每周转化率为什么是纯前端聚合、建议文案为什么不接 LLM（讨论/实现于 2026-08-22）**
最初设计是按关键词/公司类型/来源站点/匹配分/是否标星做5维度拆分表，用真实数据做了一版界面预览给用户看后，反馈"没有太大价值"，改成精简版：一行式"最近一周"汇总 + 自投递开始的周趋势柱状图 + 一段建议。`jobs` 表已有的字段（`application_status`/`applied_at`/`starred`）在首页 `allJobs` 里本来就是全量加载的，跟 `updateLede()`/`.stat-grid` 同样的既有惯例——前端拿到手的数据直接现算，不新增接口、不新增数据库字段。建议文案是 `generateWeeklySuggestion()` 里几条阈值判断（样本量、标星转化率差距、本周婉拒次数），不接 LLM——这几条是简单的规则判断，跟"偏好档案"那种真正需要语言理解归纳的分析不是一回事，接 LLM 是杀鸡用牛刀，还要多绕一层后端接口。

**周趋势图里，没有 `applied_at` 时间戳的旧记录为什么不用 `first_seen` 兜底（讨论于 2026-08-22）**
最初版本按"`applied_at` 缺失就退回 `first_seen`"分桶，结果把一条早期记录（投递状态跟踪上线前就已经手动标成"已投递"的 Amazon 职位）单独分到了一个比真实投递更早、样本量只有1条的虚假周份，用户看了预览后指出这个周实际没有投递、不该单独存在。改成：没有 `applied_at` 的记录一律并入**目前已知最早的真实投递周**（`computeWeeklyConversion()` 里 `earliestWeekStart` 取所有有时间戳记录里最早的一个），不再用 `first_seen` 单独起更早的周。

**「最近一周」为什么用滚动7天，趋势图为什么用自然周分桶——两种口径为什么都留（讨论于 2026-08-22）**
「最近一周」要贴近"现在"（用户问"这周投得怎么样"，答案应该实时滚动），改成按周一对齐的自然周会出现"周二问，只统计到当天"的割裂感；趋势图则相反，要按固定边界分桶才能稳定累积成一根根可比的柱子，用滚动窗口做不出"历史上第几周"这种离散的柱子。两个需求本质不同，统一成一种口径会顾此失彼，所以两套逻辑并存（`WEEKLY_CONV_MS` 滚动窗口 vs. `_weekStartOf()` 自然周对齐），代码里各自独立计算，不共用同一次分组结果。

**「投递分析」改造成折线图后，为什么给「收藏」「面试中」专门加时间戳字段，而不是用现有字段近似（讨论/实现于 2026-09-08）**
`jobs` 表原本只有 `first_seen`（入库时间）和 `applied_at`（首次变成"已投递"的时间）两个时间戳，`starred`（是否标星）和 `application_status`（含"面试中"）都只是当前状态的快照，没有"哪一周发生的"这个信息。改成折线图后需要"每周新增了几个收藏""每周有几个进入面试中"，有两个选项：①不改数据库，用"职位的 first_seen/applied_at 所属周 + 当前状态快照"去近似（比如"这批本周新入的职位里现在有几个标了星"），零成本但含义是"现在的状态"而不是"那一周发生的事"，旧数据容易失真；②加 `starred_at`/`interview_started_at` 两个新时间戳列，在 `set_job_starred()`/`set_application_status()`（全项目里改这两个字段的唯一入口，包括 `reconcile_application_status_from_linkedin()` LinkedIn 核对推进）里从别的状态第一次变成这个状态时刷新时间戳，代价是上线前就已经发生的星标/面试推进不会出现在图上，只能从上线那天起累积。跟用户确认后选了②——折线图存在的意义就是看"趋势"，如果连"这一周发生了什么"都不准，趋势本身就没有意义，近似口径省下的一点开发成本换不回这个准确性。两个新时间戳字段跟 `applied_at` 一样只做"跟踪辅助"用途，不影响 `starred`/`application_status` 本身的判断逻辑。

**「投递分析」折线图为什么用手写内联 SVG，而不是引入图表库（2026-09-08）**
项目前端从来没有引入任何构建工具或第三方 JS 依赖（`static/*.js` 都是纯 `<script src>` 直接引用），四条线、几十个数据点的量级也远够不上专门引入一个图表库的门槛。改用内联 SVG（`renderWeeklyLineChart()`）延续这个零依赖惯例，颜色/图例/端点标注走 dataviz 技能的分类配色方法论（固定色相顺序 + `validate_palette.js` 校验过 CVD 可辨识度），不是随手取色；四个系列色单独定义成 `--chart-*` CSS 变量，没有复用 `--primary`/`--success`——这两个在当前配色版本里都是绿色系，四条线放一起会撞色，专门给图表开一套色板反而更简单。

**「投递分析」为什么把折线图拆成四宫格小图，而不是共享一个坐标系/用对数轴（2026-09-08）**
四条系列量级差太多——「新入」几十到几百，「收藏」「面试中」经常是个位数，共享一个线性 Y 轴时后者会被压平成一条直线看不出趋势。考虑过双轴（两个 Y 轴刻度）和对数轴两个替代方案：双轴是 dataviz 技能明确列为反模式的做法（"Never a dual-axis chart"，两个刻度容易让读者把不同系列的线错误地当同一把尺子比较）；对数轴能把大小数值都塞进同一张图，但计数类数据（"这周有几个"）在对数轴上不直观，0 值也没法画（log(0) 未定义），跟"一眼看懂"的诉求不符。改成四张各自独立 Y 轴的小图（small multiples，`renderSparklineTile()`）——这也是 dataviz 技能里"量级不同的两个指标"给出的推荐做法之一，每张图只有一个系列，标题文字本身就是身份标识，不需要再画共享图例。

**「收藏」「面试中」两个新时间戳字段为什么对已有历史记录做一次性回填，而不是严格保持"只从上线起累积"（2026-09-08）**
加 `starred_at`/`interview_started_at` 时本来的设计是"只从上线那天起准确累积，历史数据不出现在图上"，这是刻意接受的代价（见上面「为什么专门加时间戳字段」那条）。上线后用截图给用户看时，「收藏」「面试中」两条线因为现有的历史标星/面试记录全部落在这个空窗期里，显示成一直是 0，用户反馈这看起来像是坏的。两个方案：严格保持原设计（新发生的才会显示，历史的永远是 0）vs. 给已经存在的历史记录做一次性回填。跟用户确认后选了后者——`init_db()` 里加两条 `WHERE ... IS NULL` 的 UPDATE，星标用 `first_seen` 兜底、面试中用 `COALESCE(applied_at, first_seen)` 兜底，天然幂等（回填过一次之后同样的 UPDATE 就是空操作），不需要额外的迁移状态表。这不是推翻"只从上线起用真实时间戳"的原则——回填只解决"上线那一刻已经存在的历史数据"这一次性缺口，之后新发生的标星/进入面试依旧走 `set_job_starred()`/`set_application_status()` 里的真实时间戳，长期统计口径不受影响。

**「面试中」为什么是区间状态（每周持续算），而不是跟「新入/收藏/投递」一样的单周事件（2026-09-08）**
最初实现把四条系列一视同仁，都按"动作发生的那一周计一次"处理（面试中按 `interview_started_at` 落在哪周分桶）。用户看过图后澄清：「新入/收藏/投递」确实该是"当周发生就算一次，之后状态怎么变都不倒扣"的单周事件——这个语义原来就是对的，不用改；但「面试中」是持续状态，进了面试没出结果之前，每一周都该算一次（比如第1周进面试、第3周还没出结果，第1-3周都要+1），原实现漏掉了这一层，只在进入那一周计数，等于把"持续中"的过程压缩成了"进入那一刻的快照"。

要表达"这段区间"，光有 `interview_started_at`（起点）不够，还得知道终点——于是加了 `interview_resolved_at`：仍处于"面试中"状态的职位没有终点，区间一直延伸到当前这一周；变成拒绝/offer/婉拒（或被手动改回其它状态）的那一刻，`interview_resolved_at` 记下终点。`set_application_status()` 因此从"单点写一次时间戳"重构成同时维护起止两个字段——进入面试中时不仅刷新起点，还要**清空**上一轮可能遗留的终点（不然一段全新的、还没出结果的面试会被上一轮的旧终点提前截断）；离开面试中时才补终点。

历史数据的局限：`interview_resolved_at` 这个字段上线之前就已经从面试中变成别的状态的记录，没法知道具体是哪一周"出的结果"，`computeWeeklySeries()` 对这类记录保守地只计入进入那一周，不外推去猜一个终点——跟 `starred_at`/`interview_started_at` 当初一次性回填时"没法完美还原历史、只能诚实反映局限"的取舍是同一个原则。
...
**打分器（`analyzer.py`）的 LLM 质量回归为什么单独成 `evals/` 目录，且用规则断言而不是黄金分数比对（2026-08-21）**
起因：`spec/product-review.md` 曾用真实使用行为得出"打分器已被证伪"的结论，核实后大部分是 LinkedIn/Indeed 同职位重复入库导致的误判——真正被用户确认的问题收窄成一类：**职级错配**（初级岗因为技能条目表面对得上，没有被 `content_match` 充分拖累）。`tests/` 下所有测试都是 `monkeypatch llm.chat`，只测周边逻辑，对 LLM 真实输出质量从来没有验证过，包括 prompt 里已经写好的"职级错配拖累"“硬性门槛拖累"这些具体规则有没有被模型真的遵守。
不做"黄金分数比对"（准备一批期望分数、跟实际输出比对）：`cognitive_match`/`content_match` 是 LLM 的主观判断，同一份 JD 换个模型甚至换一次调用都可能有正常的合理浮动，锁定绝对分数只会不断产生误报。改成两类可以客观检验的断言：① prompt 里有明确数值的规则（硬性门槛拖累：单条强制要求未覆盖 `cognitive_match` ≤0.5，两条以上 ≤0.4），直接断言绝对上限；② 没有具体数值、只有方向性描述的规则（职级错配拖累、偏好档案软信号），用**成对 fixture**（同一份简历 + 技能条目相同的 JD，只改职级/年限或有无偏好档案）比较两次调用的相对差值，断言"该更低"而不是"该等于多少"。另外顺带覆盖了 `classify_companies()` 的公司归属分类（对知名公司有客观正确答案）和公司简介编造检查（软断言，只提示不影响 exit code，因为"文本是否像编的"没法用简单规则完全判定，需要人工在报告里最终确认）。
物理上独立于 `tests/`，不接入 `tests/run_all.py`：这套测试真实调用 LLM（不 mock），产生真实 API 费用、跑起来慢（几条 fixture × repeats 次调用，每次几十秒到一两分钟），不能像 `tests/` 那样每次改动代码都自动跑一遍。`evals/run_analyzer_eval.py` 运行前会打印预计调用次数并要求手动确认（`--yes` 跳过），`--repeats`（默认2）允许重复调用同一 fixture 以过滤单次抽样的随机噪声——硬性门槛这类有明确数值的规则要求全部重复都达标，成对比较类规则按多次结果的均值判断。每条 fixture 完整的原始 LLM 输出（不只是断言结果）都会写进 `evals/reports/<timestamp>.md`（已 gitignore），因为断言只能验证"有没有遵守写死的规则"，分数背后的理由是否真的合理，仍然需要人工抽查——跟 `product-review.md` 当初核实"11条被忽略"的方式是同一个精神：不能只信一个汇总数字。

**「同步 LinkedIn 收藏/已投递」为什么复用「添加链接」的入库逻辑，只补"找齐链接"这一步（2026-08-22）**
LinkedIn 的 jobs-tracker 列表页（`jobs-tracker/?stage=saved` 已收藏、`jobs-tracker/?stage=applied` 已投递）要登录才能看，这是它跟 `job_link.py` 已支持的公开职位页唯一的本质区别——抓到详情之后要做的事（解析/去重/入库/排队分析）完全一样。所以 `linkedin_tracker.py`（这个功能最初只做"已收藏"，模块名叫 `linkedin_saved.py`；同一天用户追加"已投递"的同样需求后，发现两个列表页结构完全一样，改成按 `stage` 参数区分并改了模块名）只负责"开一次登录态浏览器，把某个 stage 的列表滚动到底，收集出现过的职位链接"，找齐链接后原样交给 `job_link.add_jobs_from_urls()`，不重新实现抓取+入库。这两个列表页的 DOM 结构都没有公开文档、也没法用访客身份打开预先看一眼（本来就要登录才能看），所以提取时刻意不认卡片的具体 class（LinkedIn 自动生成的哈希类名，大概率随时间/AB测试变化），只认页面上所有指向 `/jobs/view/<id>` 或带 `currentJobId=` 的链接——这两种形态 `job_link.parse_linkedin_job_id()` 本来就要处理，直接复用判定逻辑，也让这个模块对页面改版的抵抗力更强（只要卡片本身还是个指向职位详情的链接，不管外层包装怎么变都能抓到）。
"已投递"比"已收藏"多一步业务语义：新入库的职位要自动标记 `application_status='applied'`。这一步刻意放在 `app.py` 的后台处理函数里做，不下沉进 `linkedin_tracker.py`——那个模块只关心"找链接、转手给 add_jobs_from_urls"，不该知道"已投递"这个 stage 在职达的数据模型里意味着什么，保持"抓取模块不掺业务状态语义"的边界，跟 `job_link.py`/`linkedin_company.py` 一直以来的分工原则一致。
同步耗时不确定（开浏览器 + 滚动加载列表 + 逐条抓详情，列表长的话要一两分钟），不能像「添加链接」那样在一次 HTTP 请求里同步等完；沿用体检（`resume_review`）、题库起草（`bank`）已有的"POST 启动 + GET 轮询状态"模式（`job_state.py` 的 `start_tracker_sync(stage)`/`finish_tracker_sync(stage, ...)`），没有再发明一套新模式。状态按 stage 分开存（`_tracker_sync = {"saved": {...}, "applied": {...}}`）而不是全局共用一份：这两个同步是用户会分别独立触发的两件事，共用一份状态会导致"正在同步已收藏"时误报"已投递也在同步中"、或者一个的结果覆盖另一个还没被前端看到的结果。真正防止不同类型的同步同时抢同一个登录 profile，2026-09-08 之前完全靠 Chromium 对 profile 目录的独占锁"谁后 launch 谁失败"（`_launch_context` 抛 `EasyApplyInProgress`）——按 stage 分离只解决了"前端状态展示不串台"，不同类型撞车时后一个直接收到含糊的报错。2026-09-08 起改成显式排队，见下面「LinkedIn 登录态浏览器排队」。

**同步 LinkedIn "How You Fit" 匹配推荐为什么要引入 agent，以及为什么只让它管"下一步点哪/滚哪"（2026-08-23）**
起因：用户想让每日定时任务顺便同步 LinkedIn 的 "How You Fit" 求职资格匹配搜索结果页（登录态下才能看，基于用户档案算出"你可能符合条件"的结果）。这跟 `linkedin_tracker.py` 同步"已收藏"/"已投递"是同一类事——开登录态浏览器扫一个列表页、把链接转手给 `job_link.add_jobs_from_urls()`——但 `jobs-tracker` 页面结构已经被验证过，How You Fit 用的 `/jobs/search-results/` 模板结构从没被验证过。照抄硬编码选择器的风险是：一旦猜错（比如实际是内嵌可滚动侧栏而不是整页滚动），代码会**静默返回空结果**而不报错，这正是本项目最近讨论"什么时候真的需要 agent"时定下的判断标准——分支太多、没法提前画出固定流程图的场景，才值得为不确定性买单。
方案是**混合模式**：`linkedin_list_scan.py`（从 `linkedin_tracker.py` 抽出来的共享扫描模块）的确定性扫描优先跑，快、不花钱；只有它一无所获、或者收集到的数量明显偏少（低于 `MIN_DETERMINISTIC_JOB_IDS`，见下方 2026-08-27 决策条目），才升级给 `linkedin_how_you_fit._scan_with_agent()` 接管——开一次浏览器，循环最多 `MAX_AGENT_STEPS`（12）轮，每轮把页面上的可交互候选元素描述给 LLM（`llm.chat_tool_step()`，只支持 anthropic），LLM 用工具调用选"点哪个/滚哪个/判定结束"，执行后重新收集职位链接，直到 LLM 判定 `reached_end`（正常结束）或 `stuck`（判断不了，直接报错，不猜）。日常运行 LinkedIn 页面结构没变时完全用不到这条路径，成本接近零；只在确定性路径失败时才会真的触发一次 LLM 决策循环。
候选元素的感知设计走过一次弯路：最初版本按"更多"/"加载"/"下一页"这类关键词预筛候选控件——复审时发现这等于把"不知道 DOM 长什么样"的问题换个形式又猜了一遍，关键词猜不中，agent 照样卡住，跟原来的硬编码选择器是同一类坑。改成结构性提取：按 DOM landmark（`el.closest('nav, header, footer, ...')`）排除站点通用"chrome"区域，不按文案关键词猜，这样才是真正解决了"没法提前枚举所有可能形态"这个根本问题。
两条安全网防止"给 agent 更大自由度"这件事反过来制造新风险：① 候选列表排除能被 `job_link.parse_linkedin_job_id()` 识别成职位链接的元素——点了会导航去职位详情页，把整个扫描带偏；这个判断刻意放在 Python 侧（不在 JS 里重复写一份正则），复用生产入库同一份解析函数，也让这条安全网能被单测直接覆盖（`_PAGE_SNAPSHOT_JS` 两阶段设计：先在 JS 里给全部候选打临时编号+带出 href，Python 筛完幸存者后再回填正式编号，不需要真起浏览器就能测过滤逻辑本身）。② 每次执行完 agent 选的动作后，检查页面的 `(scheme, host, path)` 是否还跟原始 url 一致（忽略 query string），一旦不一致，判定被导航离开、立刻中止——这是候选过滤没兜住时的第二层防线。候选列表还设了 80 项的展示上限（按页面位置截断），控制单轮 token 成本。
`llm.py` 新增 `chat_tool_step()` 只支持 anthropic，不支持 DeepSeek：这是项目第一次需要工具调用能力，DeepSeek 的工具调用协议跟 Anthropic 不同，而目前唯一消费者只需要一个 provider 能跑起来，统一两家协议在这个阶段是不成比例的投入，需要时再扩展。

**LinkedIn How You Fit 同步为什么要覆盖"不做个性化推荐流自动化抓取"的决策，以及具体怎么兜底风险（2026-08-23）**
...
**How You Fit 升级给 agent 兜底的判断条件为什么从"抱空"放宽成"数量偏少"（2026-08-27）**
用户反馈实测下来 How You Fit 每次只同步到很少几个职位，怀疑是滚动没生效。排查确认：`linkedin_list_scan.py` 找不到"加载更多"按钮时会退化成 `page.mouse.wheel` 滚整个窗口，但 How You Fit 页面大概率是左侧职位列表单独一个可滚动容器，滚窗口滚不到那个容器，导致最初渲染的几个职位收集完后很快连续 3 轮无新增、被判定"到底"提前停手——这正是 `_scan_with_agent()` 设计初衷要覆盖的情况（它会扫描页面找真正的可滚动容器），但原判断条件 `if not job_ids` 要求确定性扫描完全抱空才升级，"扫到一点但不全"根本走不到这条兜底路径，agent 已经写好的能力被判断条件挡住用不上。
修法只调整了触发条件（`len(job_ids) < MIN_DETERMINISTIC_JOB_IDS`，阈值定为5）和结果合并方式（并集去重，不再让 agent 结果替换掉确定性结果），没有改动 agent 兜底本身的候选提取/可滚动容器识别逻辑——那部分从 2026-08-23 上线起就没变过，这次要解决的是"该用的时候没用上"，不是"用了但不管用"。代价是数量本来就正常偏少的搜索也会多花一次 agent 调用（多一次 LLM 请求成本），但 `MAX_HOW_YOU_FIT_SEARCHES` 已经把每日总次数卡住了，可接受。
...
**通知功能为什么是持久化 DB 表 + 前端轮询，而不是 WebSocket/SSE 服务端推送（2026-08-26）**
后台任务（同步/AI分析）完成时要让 6 个页面的顶栏铃铛都能看到未读提醒，两条路：服务端主动推（WebSocket/SSE，任务一完成立刻推给所有打开的标签页）或者前端定期拉（新增 `notifications` 表落一行，前端隔一段时间查一次未读数）。选了后者：这是本地单进程 Flask 开发服务器（`app.run()`，不是生产级 ASGI/WSGI 部署）、单用户使用，WebSocket/SSE 要新增长连接管理和一套推送订阅机制，维护成本对这个场景不成比例；轮询的代价只是最多 25 秒的延迟（`static/common.js` 的 `initNotifications()`），而通知本身不是需要秒级送达的场景（对比：批量分析动辄几分钟，多等 25 秒不影响使用）。落 DB 表而不是复用 `job_state.py` 那套进程内状态，是因为通知需要跨进程重启保留（用户关掉终端重开、或者服务重启，之前错过的通知不该跟着消失）——这跟 `job_state.py` 里各种 `queued/generating` 状态"重启后重新触发即可、不需要持久化"的假设不一样：那些是"任务进行中"的瞬时状态，这里是"任务发生过"的历史记录。

**防幻觉分层：做什么、不做什么（2026-08-29）**
起因是一份流传的"五层防幻觉体系"（Prompt 层 / RAG 层 / 生成控制层 / 后验证闭环层 / 可观测性层）。这个分层本身是合理的思路框架，但套到本项目要逐层判断，不能照单全收——下面每一条"不做"都是决策，不是遗漏。

- **第1层 Prompt：已有，维持现状。** 四个 LLM 模块的 PROMPT_TEMPLATE 里各自都写了"不要编造"，措辞刻意不统一（专用措辞比通用文案更精确，见 2026-08-29 那条"LLM 功能层收敛共享工具"的决策）。
- **第2层 RAG：不做。** 本项目所有 LLM 调用都是"单文档进单文档出"——JD 配简历、单职位问答、整份简历体检，输入是一份明确的文档，不存在"从大语料库里找相关内容"的检索问题。直接把结构化数据拼进 prompt 是对的做法，不是偷懒。唯一有语料形态的是 `interview.build_bank_block()`（全量注入题库，每条答案截 500 字），但目前只有几十条，全量注入完全扛得住。**触发条件**：题库涨到几百上千条、必须截断到答案失真时，再给这一个功能单独接 embedding 检索，不需要为了框架完整性整体引入 RAG。
- **第3层 生成控制：做了一半，且不是为了防幻觉。** 降温管的是输出稳定性和可复现性（同一份 JD 分析两次分数别差太多），对"事实性幻觉"基本无用——模型该编还是会编，只是编得更整齐。真正的约束是：**`claude-sonnet-5` 已从 API 移除 `temperature`/`top_p`/`top_k`，传了直接 400**，而它是本项目的默认模型。所以做法是先加 `MODELS.supports_sampling` 开关（防 400 是纯收益），温度值本身是保守初值、还没用 eval 量过。Sonnet 5 上真正对症的是结构化输出（`output_config.format`，能直接消灭 `extract_json` 解析失败那一类），但那个要按任务试点、别一次铺开，暂缓。
- **第4层 后验证：这是本项目的主要路线，ROI 最高。** 核心手法是**用确定性代码核查模型的事实性断言，而不是用另一个模型核查**。两个落地点：`resume_edits.py` 把模型声称"照抄"的段落原文回简历里比对；`analyzer.verify_mandatory_items` 把模型声称"JD 里写了 required"的原话回 JD 里比对。两者都是纯 Python、零 LLM 成本、结果确定，而且都对着真实输入验证，不是让模型自我审查。
- **验证器模型二次核查 / 置信度评分：不做。** 前者成本和延迟都翻倍，而本项目的核心幻觉风险（段落引用错位、把 preferred 说成 required）用上面那种确定性核查就能抓到，用模型查模型反而引入第二个会编造的环节。后者的问题是 LLM 自报置信度校准很差（普遍 overconfident），要真有效需要独立验证器——绕回前一条。项目里已有的 `company_origin: "unknown"`、"如实说未找到"这类"允许说不知道"的设计，才是这个方向上低成本且有效的做法。
- **第5层 可观测性：做了，而且先做。** 理由是没有度量就说不清前面几层有没有用——改完温度、加完核查，失败率降了没有、成本涨了没有，此前一个都答不上来（`llm.py` 连 `import logging` 都没有，Anthropic 的 `resp.usage` 从未被读取）。

**`llm_calls` 埋点为什么用 ContextVar 传 task，而不是给 `chat()/ask()` 加参数（2026-08-29）**
流水要按功能位聚合（"哪个任务最容易失败"），但 `llm.py` 的四个对外入口不知道调用方属于哪个功能位——`resolve_task(cfg, task)` 知道，它在 `pipeline.py` 里被调 12 次，每次都紧接着就要调 LLM。三个方案：给入口加 `task=` 参数（要改 4 个入口 + 各模块约 10 个函数签名 + 12 个调用点，**并且会打断 6 个测试文件**——`test_linkedin_tracker_sync.py` 那些把 `llm.chat` 换成固定签名 lambda 的 monkeypatch，多一个 kwarg 就 TypeError）；用 `with llm.task_context(...)` 包住 12 个调用点（不改签名但 diff 不小）；或者让 `resolve_task()` 顺手把 task 写进 ContextVar（零调用点改动、零测试改动）。选了第三个。代价是一个 getter 带了副作用，这是明确认下来的坏味道——前提是"调 `resolve_task` 就等于马上要调 LLM"，目前 12 个调用点全部成立；如果将来有地方只想知道用哪个模型并不真的要调（比如设置页回显），标签会脏。唯一不走 `resolve_task` 的 LLM 路径是 `linkedin_how_you_fit.py` 的 agent 兜底，那里显式用了 `task_context("how_you_fit_agent")`。这个 ContextVar 后来被第3层的 `temperature_for()` 复用了，这也是第5层先落地的直接回报。

**`llm.py` 为什么用回调注册写流水，而不是直接 `import models`（2026-08-29）**
`llm.py` 是全项目唯一不依赖任何项目内模块的 LLM 适配器（见 [architecture.md](architecture.md) 第5层），直接 import 会造出"第5层地基反向依赖第4层数据层"。所以 `llm.py` 只留 `set_recorder(fn, error_updater)`，由 `app.py` 启动时注册 `models.insert_llm_call`。代价是忘了注册就悄悄没数据——`tests/test_llm_logging.py` 里有一条断言直接读 `app.py` 源码盯着这行。另一个附带好处：把 `llm.chat` 整个 mock 掉的测试根本走不到埋点，测试环境天然不写库，不需要额外隔离。

**LLM 用量展示为什么再开一个 ContextVar 累计"一次操作花了多少"，而不是按时间窗口查 `llm_calls`（2026-09-08）**
把已有的 `llm_calls` 流水在网页端展示出来时（顶栏花费小组件 + toast/通知里的"这次花了多少"），单次操作那部分本可以按"这次请求开始之后新增的行"反查 `llm_calls`，但本地虽然单用户，Flask 开发服务器是多线程的，并发请求之间没有天然的时间窗口隔离，按时间戳筛容易筛串，而且路由和后台线程两类调用方都得各自拼一遍筛选逻辑。改成照抄 `_current_task` 那条 ContextVar 的路子（见上面"`llm_calls` 埋点为什么用 ContextVar 传 task"）：`llm.start_usage_tracking()`/`pop_usage_summary()` 直接复用 `_CallRecord.__exit__` 里已经算好的 `input_tokens`/`output_tokens`/`cost_usd`，append 进 ContextVar 存的列表，不再碰数据库；Flask 每个请求、`threading.Thread` 每个后台任务都是独立线程、独立 context，天然互不串味，不需要显式加锁或清理。代价是这份汇总只活在本次调用的进程内存里、不落库——可以接受，因为完整历史已经在 `llm_calls` 表里了，这里只是给这一次操作递一张"回执"，不是账本本身。

**DeepSeek 定价为什么只存一个数，不精确建模 cache hit/miss + 高峰/非高峰四档（2026-09-08）**
`llm_calls` 流水上线时（2026-08-29）DeepSeek 两个模型一直没配 `price_in`/`price_out`，成本栏一直是 `NULL`。查了 DeepSeek 官方定价页（[api-docs.deepseek.com/quick_start/pricing](https://api-docs.deepseek.com/quick_start/pricing/)，价格实际画在一张图里，文本抓取工具读不出来，是把图片下下来直接看才拿到数字的）后发现它比 Anthropic 复杂一截：区分 cache hit 和 cache miss 两档输入价（Flash 非高峰分别是 $0.007/$0.22 每百万，Pro 是 $0.022/$0.66），还叠加 UTC 高峰（01:00-04:00、06:00-10:00）/非高峰两档、高峰价是非高峰的 2 倍——真要按实际情况精确算，`estimate_cost()` 得知道每次调用有多少 token 命中了缓存（DeepSeek usage 里有这个字段，`_deepseek_usage()` 已经原样存进 `usage_json`，字段本身没丢）、以及调用发生在 UTC 哪个时间段，这已经不是加两个数字的事，是给 DeepSeek 单独写一套跟 Anthropic 完全不同形状的定价函数。当前的 `estimate_cost()`/`MODELS` 结构（每个模型一对 `price_in`/`price_out`）从设计上就没打算装下这种多档定价，本来也只是"估算不是账单"（见上面「防幻觉三层加固」条目里的说明），不值得为了这一家 provider 重新设计整个计价接口。选择只存**非高峰、cache miss** 这一档：非高峰占一天 17/24 小时，是更常见的情况；cache miss 是没命中缓存时的价，用它不会把估算算得比实际还低（命中缓存时真实花费只会更少，不会更多）——跟 Anthropic 那边"不算 `cache_read_input_tokens` 折扣价，按满价的 `input_tokens` 算"是同一个"宁可估高不要估低"的取舍方向，两家保持一致。代价：DeepSeek 常年跑批量任务、缓存命中率高的话，这里显示的成本会持续偏高，用户如果真的要精算账单，`usage_json` 里原始的 `prompt_cache_hit_tokens` 数据还在，回头可以重算，不算丢失信息。

**硬门槛封顶为什么要求模型照抄 JD 原话，而不是直接信它的 `is_mandatory`（2026-08-29）**
`apply_score_rules()` 会把"未覆盖的强制性要求"对应的 `cognitive_match` 从 0.85 压到 0.3——这是个杀伤力很大的规则，判断错了比不做还糟：模型只要把 JD 里的 "preferred" 也标成强制，大批职位就会被误杀。所以 `requirement_items` 要求同时给 `mandatory_evidence`（JD 原文里标注强制性的那一小段原话，逐字照抄、不许翻译转述），程序归一化后回 `jd_text` 里核对，**核不过就把 `is_mandatory` 当 False**（fail-open，退回没有这条规则时的行为）。这条规则能不能真正生效，取决于核验通过率——通过率低就说明模型在转述而不是照抄，那时应该只记录不封顶。`evals/run_analyzer_eval.py` 会报每次的核验通过条数。另外 `is_mandatory`/`mandatory_evidence` 刻意**不进** `validate_analysis_result()` 的必填校验：模型偶尔漏字段就让整条分析抛错，为一条兜底规则牺牲主流程可用性不划算。

**LinkedIn 登录态浏览器排队：为什么是显式 FIFO 队列，而不是继续靠 OS 锁"撞上就报错"（2026-09-08）**
起因：用户反馈「同步 LinkedIn 智能匹配推荐全部搜索」每次都抓不到内容，排查发现是同一时刻还有另一个同步（收藏列表）占着登录态浏览器，`sync_search()` 直接抛 `HowYouFitSyncError`（"有另一个 LinkedIn 浏览器窗口正在用同一个登录 profile"）——tracker 同步（收藏/已投递/面试）、How You Fit 单条/批量同步四类操作之前完全没有跨类型协调，`job_state.py` 里 `_tracker_sync`/`_how_you_fit_sync`/`_how_you_fit_batch` 各自的 `syncing` 标记只挡"同一类型重复点"，不同类型撞车全靠 Chromium 对 profile 目录的独占锁"谁后 launch 谁失败"，用户体验是"莫名其妙报错、通知还说成功"（后者是同一次排查里修的另一个独立 bug，见 roadmap.md）。
用户明确要求排队方案（而不是"排到就立刻报清晰的错，让用户自己重试"）：`job_state.py` 新增 `acquire_linkedin_browser(label, start_fn)`/`release_linkedin_browser()`，一个进程内共享的 FIFO 队列 + 占用标签。占用中的调用方把 `(label, start_fn)` 存进队列；占用者跑完调用 `release_linkedin_browser()`，如果队列非空，直接在新线程里跑队首的 `start_fn`——不需要用户重新点按钮。`label` 复用各功能已有的中文提示词（"同步 LinkedIn 收藏列表"之类），排队方给用户的提示直接引用它，不需要另外维护一套"操作类型"枚举去反查文案。各自的 `queued_behind` 状态字段（跟 `syncing`/`result`/`error` 放一起）让前端轮询接口能区分"正常同步中"和"排队等 XX"，不是只有一个含糊的 `syncing: true`。
刻意排除 Easy Apply 和「贴链接」的浏览器兜底：前者是"打开浏览器窗口留给用户自己点提交"的半人工流程，没有程序能感知到的"结束"时刻（不会自动 close，等用户自己关窗口），排在它后面的自动同步会不知道要等多久甚至永远等不到；后者是一次 HTTP 请求内同步等完的（`add_by_url` 的设计取舍见上面「已收藏/已投递」章节的说明），接入排队会让这次请求挂着不知道多久返回。这两个仍然用原来的"撞上就报错"（各自独立的 `easy_apply_opening()`/OS 锁），跟这次要解决的场景（两个全自动后台同步按钮互撞）不是同一类问题。
每日定时任务（`scheduler.py` 直接调 `sync_all_enabled_searches()`）目前没有接入这套队列——它在自己独立的后台线程里同步跑完整个批次，跟用户当前请求路径没有共享调用点，接入需要把 `_run_job()` 也拆成"排队等浏览器 → 回调式继续"的异步结构，这次没有一并做（用户报告的场景是两个手动点的按钮撞车，不是定时任务撞车），记在这里避免以后误以为定时任务已经全覆盖。
...
- xlsx 当"数据库"用没有并发写保护，目前单进程场景下不是问题，多进程同时写会有风险——2026-08-22 这个风险真实发生过一次：测试套件里几个测试文件的追踪表路径意外留空、解析成了真实的 `~/Downloads/JD匹配追踪表.xlsx`，跟当时正在跑的真实 `app.py` 撞车写坏了这个文件（详见 [roadmap.md](roadmap.md#修复测试套件曾经写坏用户真实追踪表文件的事故2026-08-22)）。这次只是堵住了"测试进程意外写真实文件"这一种触发方式，`tracker_utils.py` 的 `wb.save()` 本身仍然不是原子写，两个真实进程（比如本机同时开着两份 `app.py`）同时写同一个追踪表文件的话，同样的损坏还会发生。
...
- How You Fit 页面结构在写代码时没有用真实登录态手动验证过（页面本身要登录才能看），agent 兜底路径按 DOM landmark（`nav`/`header`/`footer`）排除站点通用区域的判断是"如果 LinkedIn 用了语义化标签就管用"的假设，没有实测确认；候选元素提取用的 accessibility/交互元素选择器同理。真正跑起来如果 agent 频繁判定 `stuck`，需要用带界面模式手动打开一次真实页面看实际 DOM 结构，按需调整 `linkedin_how_you_fit.py` 的候选提取逻辑。
