...
### 公司简介聚焦到具体部门/业务线（讨论于 2026-08-18）
起因：像亚马逊这类大公司，JD 里通常会提到具体部门/业务线（如某个 BU），但现有 `company_overview` 只按"整个公司"写简介，信息太笼统，帮不上"这个具体岗位到底在做什么业务"的判断。
- 讨论过要不要接联网搜索去查这个部门的信息：调研确认 DeepSeek 官方 API 不带联网搜索，需要额外接第三方搜索 API（如 SerpAPI）或把这一步单独切到 Anthropic 的原生 web_search 工具，两种都要多一层依赖/成本
- 用户决定先不接真实联网搜索，只调整 `analyzer.py` 的 `PROMPT_TEMPLATE`：如果 JD 提到具体部门/业务线，优先让模型基于自身知识描述这个部门/业务线在做什么，而不是笼统写整个公司；没提到具体部门时维持现状（写整个公司）
- 这仍然是单轮 prompt 优化，不涉及工具调用/自主决策，不算 agent 化，跟上面"AI agent 化探索"是两回事
- 尚未实现，待后续排期
...
---
最后更新：2026-08-18（新增"AI agent 化探索"、"公司简介聚焦到具体部门/业务线"两条讨论）
