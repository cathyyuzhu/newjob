...
用户提出"本地开发的项目，出门在外用手机怎么继续开发"，方案是 Claude Code on the web/手机 app——每次开新 session 是全新云端容器，只 clone 代码，`config.json`/`jobs.db`/`resumes/`/API key 等本地专属数据和密钥都不在里面（本来就该如此，不建议同步真实数据到云端）。这次单独解决"每次新 session 都要手动 `pip install -r requirements.txt` 才能跑测试"的问题：新增 `.claude/hooks/session-start.sh`，只在 `CLAUDE_CODE_REMOTE=true`（云端容器）时执行，建一个 `.venv`（容器系统 Python 由 apt 管理部分包缺 RECORD 文件，直接装到系统环境会报错，改用虚拟环境隔离）并装好 `requirements.txt`，通过 `$CLAUDE_ENV_FILE` 把 `.venv/bin` 加进后续所有 bash 命令的 `PATH`。在 `.claude/settings.json` 里注册为 `SessionStart` hook（同步模式，不用 async——保证依赖装完了 session 才真正开始，避免用测试/lint 时依赖还没装好的竞态）。验证过 `tests/test_llm.py` 单条用例和 `node --check` 都能在 hook 跑完后直接执行。这个 hook 要合并进默认分支之后，以后所有手机端 session 才会自动生效。涉及文件：`.claude/hooks/session-start.sh`（新增）、`.claude/settings.json`。
