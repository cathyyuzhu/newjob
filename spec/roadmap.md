...
### 「已收藏」列表补上 AI 分析按钮（2026-09-09）
- 修复：「已收藏」筛选下、还没有分析结果（`overall_match` 为空，比如手动粘贴链接直接标收藏、或分析失败/JD 未获取）的职位卡片，原来因为 `j.status === 'reviewed' ? '' : analysisStateButtonHtml(j)` 这行直接把分析按钮隐藏了，导致这类卡片无法从列表页发起分析、只能进详情页操作。改成按有没有分析结果判断，而不是按审核状态判断（`static/app.js` 的 `jobCardHtml()`）
...
- 用法见 [README.md](../README.md#投递分析怎么看)（原"每周转化率怎么看"，随下面 2026-09-08 改造一并改名）；涉及文件：`templates/index.html`（`.today-row`/`#weeklyConvCard`）、`static/app.js`（`computeWeeklyConversion()`/`renderWeeklyConversion()`/`generateWeeklySuggestion()`）、`static/style.css`
...
- **改造为四线折线图（讨论/实现于 2026-09-08）**：用户反馈文字汇总和规则式建议没有必要，只要看"新入/收藏/投递/面试中"四条线按自然周的趋势就够，其余信息全部去掉——`computeWeeklyConversion()`/`generateWeeklySuggestion()`/汇总文字/标星-未标星对比/婉拒提醒整段删除。「收藏」「面试中」原本只是布尔值/状态字符串快照，没有"哪一周发生"的时间戳；确认过用"所属周+当前状态快照"近似 vs "先加时间戳字段再做图"两个方案后，用户选了后者——`models.py` 加 `starred_at`/`interview_started_at` 两列（`set_job_starred()`/`set_application_status()` 里跟 `applied_at` 同样的写入时机：从别的状态第一次变成这个状态时刷新时间戳），代价是上线前就已经标星/进入面试的历史数据不会出现在图上，只能从上线那天起准确累积。折线图是内联 SVG（项目零依赖风格，没引入图表库），四条系列色走 dataviz 技能的分类配色方案（`validate_palette.js` 校验过 CVD 可辨识度），新增 `--chart-new/--chart-starred/--chart-applied/--chart-interviewing` 四个 CSS 变量，不复用 `--primary`/`--success`（色相太接近，四条线放一起会混）
- 涉及文件（本次改造）：`models.py`（`init_db()` 加列、`set_job_starred()`、`set_application_status()`）、`static/app.js`（`computeWeeklySeries()`/`renderWeeklyLineChart()`/`renderWeeklyConversion()`）、`templates/index.html`（`#weeklyConvCard` 内部结构精简）、`static/style.css`（新增折线图样式和四色 token）
- **拆成四宫格小图 + 历史数据一次性回填（讨论/实现于 2026-09-08，同日追加）**：截图给用户看四线折线图效果后发现两个问题——①「新入」量级（几十到几百）比其它三个指标（个位数到十几）大一个数量级，共享同一个 Y 轴时后三条线被压平成直线看不出趋势；②「收藏」「面试中」两条线显示一直是0，查库确认不是 bug——现有的2条已标星、3条面试中记录都是在 `starred_at`/`interview_started_at` 上线前就存在的历史数据，时间戳是空的，这正是加时间戳字段时讨论过并接受的代价，但用户看到"一直是0"仍然觉得像是坏的。两个问题都跟用户确认过解法：①拆成四张各自独立 Y 轴的小图（small multiples，`renderSparklineTile()`），不再共享坐标系；②给 `init_db()` 加两条一次性回填 UPDATE（`WHERE ... IS NULL` 保证天然幂等）——已标星的记录 `starred_at` 回填成 `first_seen`，面试中的记录 `interview_started_at` 回填成 `COALESCE(applied_at, first_seen)`，只补历史缺口，之后新发生的仍走真实时间戳。回填后用 Playwright 验证「收藏」序列变成 `0,0,1,1,0`、「面试中」变成 `0,1,2,0,0`，跟库里的真实记录（id 234/572 标星，id 69/203/234 面试中）完全对得上。删掉的：共享折线图专用的 `renderWeeklyLineChart()`、`_deoverlapLabels()`（防标签重叠，小图每张只有一个系列不存在这个问题）、共享图例 `#weeklyConvLegend`（小图各自标题带色点，不需要图例框）。涉及文件：`models.py`（`init_db()` 回填 UPDATE）、`static/app.js`（新增 `renderSparklineTile()`，删除 `renderWeeklyLineChart()`/`_deoverlapLabels()`，重写 `renderWeeklyConversion()`）、`templates/index.html`（删 `#weeklyConvLegend`）、`static/style.css`（图表样式从共享坐标系整套换成 `.wc-tile` 系列）
- **「面试中」改成区间状态而不是单周事件（讨论/实现于 2026-09-08，同日再追加）**：用户看过四宫格小图后澄清真实诉求——「新入/收藏/投递」是"当周发生了这个动作就算一次"的单周事件，之后职位状态怎么变都不影响已经记下的这一周（收藏了之后投递、投递了之后面试，都不该从原来那周被扣掉，这点当时已经这么实现了）；但「面试中」不一样，是持续状态：进了面试还没出结果，每一周都该算一次（比如第1周进面试、第3周还没出结果，第1-3周各自都要+1），不是只算进入那一周。原实现把「面试中」当成跟其它三个一样的单周事件，没法表达"持续中"。加了 `interview_resolved_at` 字段记录"离开面试中状态"的时间点，`set_application_status()` 从单点写法重构成同时维护面试区间的起止：进入"面试中"时刷新 `interview_started_at` 并清空 `interview_resolved_at`（全新一轮还没出结果的面试，不能被上一轮遗留的解决时间戳提前截断）；离开"面试中"（变成拒绝/offer/婉拒/甚至手动改回待投）时刷新 `interview_resolved_at`。`computeWeeklySeries()` 对「面试中」单独处理：从 `interview_started_at` 所在周循环加到"出结果"那周（含）——仍在面试中就一直加到当前这周；`interview_resolved_at` 上线前就已经解决的历史面试没有这个时间戳，不知道具体哪周出的结果，保守地只计入进入那一周，不外推。用真实数据核对：3 条现有面试中记录都还没出结果，改造前「面试中」序列是 `0,1,2,0,0`，改造后变成 `0,1,3,3,3`，当周数值从"卡在0"变成正确的"3"。涉及文件：`models.py`（`init_db()` 加 `interview_resolved_at` 列、重写 `set_application_status()`）、`static/app.js`（`computeWeeklySeries()` 对 `interviewing` 系列改成区间累加）
...
- **追加（2026-09-08）：通知/toast 补上"具体更新了哪几条"的明细**——用户反馈同步完成只报"投递状态核对更新 2 条"，不知道是哪两条职位、从什么状态变成什么状态，看不出这个数字对不对劲。`reconcile_application_status_from_linkedin()` 返回值从纯计数（int）改成明细列表（每条 `{id, title, company, application_status_before, application_status_after}`），`len()` 就是原来的计数，调用方不用再单独维护一份。`linkedin_tracker.sync_tracker_stage()` 收尾时把明细列表原样透传（`result["reconciled_details"]`），`routes_search.py` 新增 `_format_reconciled_note()` 把明细拼成"公司·职位名：旧状态→新状态"（只被 `_promote_reviewed_for_applied_jobs()` 顺带推进 `status`、这次调用 `application_status` 本身没变的条目，标"标记为已审核"而不是"旧状态→旧状态"），最多列 5 条，超过折成"等 N 条"；`static/app.js` 新增同名逻辑的前端版本 `formatReconciledNote()`，跟后端文案保持一致。涉及文件：`models.py`、`linkedin_tracker.py`、`routes_search.py`、`static/app.js`、`tests/test_reconcile_application_status.py`（返回值断言从 `updated == N` 改成 `len(updated) == N`，并新增一条明细字段内容校验）。
...
### LinkedIn 智能匹配推荐同步补上标题粗筛（2026-09-04）
- 起因：用户反馈点「智能搜索」后，通过「LinkedIn 智能匹配推荐」（How You Fit）同步进来的一些职位标题明显不符合搜索方向。排查确认是设计缺陷：`linkedin_how_you_fit.sync_search()` 一直沿用"LinkedIn 自己判定'符合资格'的结果，不该被关键词粗筛二次质疑"这条理由（跟 tracker 同步收藏/已投递/面试列表共用同一套不筛选逻辑），但 tracker 那三个列表是用户自己在 LinkedIn 上一条条操作过的，How You Fit 是算法按搜索链接的 `keywords=` 参数批量推荐的候选，两者信任基础不一样——算法并不总是老实按标题匹配。
- 修复：`job_link.add_jobs_from_urls()` 新增可选的 `keyword` 参数，传了就在入库前用 `relevance.title_looks_relevant()` 挡掉标题跟这个关键词完全不沾边的结果（标记成新状态 `skipped_irrelevant`，不插入库、不占 `added_ids`）；不传（手动贴链接、tracker 三个 stage 的调用方）保持原样不筛，不影响那些"用户自己挑的/已操作过的"场景。`linkedin_how_you_fit.py` 新增 `_extract_keyword(url)` 从搜索链接的 `keywords=` 查询参数里取出真实关键词，`sync_search()` 用它调用 `add_jobs_from_urls`；同步结果/通知文案（`routes_search.py` 两处 How You Fit 完成通知）补上"标题不符跳过 N 条"提示，让筛选效果对用户可见。地点（`geoId=` 是 LinkedIn 内部数字地理编码，没有现成映射表还原成城市名）这次不筛，范围只覆盖用户报告的标题问题。
- 全量测试套件（30 个套件 + 7 个 JS 语法检查）复跑通过。
- 涉及文件：`job_link.py`、`linkedin_how_you_fit.py`、`routes_search.py`、`tests/test_linkedin_how_you_fit_sync.py`

### LinkedIn 智能匹配推荐「全部搜索」批量同步的失败被通知悄悄吞掉（2026-09-08）
- 起因：用户反馈「同步 LinkedIn 智能匹配推荐全部搜索」这个功能每次都抓不到内容，但通知栏一直显示"同步完成"、"失败 0 条"，看起来像是成功了，用户没法从界面上判断到底哪里出了问题。
- 排查：查 `collect_runs` 表发现当前唯一启用的那条搜索这次同步确实失败了——`sync_search()` 抛出 `HowYouFitSyncError`（"有另一个 LinkedIn 浏览器窗口正在用同一个登录 profile，请等它结束后重试"，大概率是用户前后脚点了「同步收藏/已投递/面试」和「同步全部搜索」两个都要开登录态浏览器的按钮，两者共用同一个 profile 目录锁，撞车了）。`sync_all_enabled_searches()` 对这种整条搜索跑失败的情况处理是对的，会把 `{"result": None, "error": "..."}` 记进 summary；真正的 bug 在 `routes_search.py` 的 `_sync_how_you_fit_all_background()`：汇总通知只统计每个 `entry["result"]["results"]` 里逐条 URL 的 `status == "failed"`，从来没读过 `entry["error"]`——一整条搜索还没开始扫就报错的情况完全没被计入，通知永远说"失败 0 条 · 成功"，把真实原因埋没了。前端 `pollHowYouFitAll()` 里当场弹的 toast 其实是对的（它直接读 `summary` 原始数据、自己算了一遍 `entry.error`），只有事后翻通知历史/铃铛才会看到这条被masked成功的记录，容易让人以为"每次都莫名其妙抓不到"。
- 修复：`_sync_how_you_fit_all_background()` 汇总时额外收集所有 `entry.get("error")` 非空的条目，通知文案追加"N 条搜索整体失败：具体原因"，且只要有整体失败的搜索就把通知级别标成 `error`（原来只看逐条 URL 失败数）。这样以后再撞上"另一个浏览器窗口占用 profile"这类整条搜索失败的情况，通知本身就能说清原因，不需要回来翻 `collect_runs` 表。
- 这次这条搜索本身失败的根因（两个都要开登录态浏览器的同步按钮前后脚点、共用同一把 OS 级 profile 锁）当时是代码里写明的已知取舍（"两者可能同时被触发时不强行互斥"），这一版只修了"失败了却看不出来"的上报缺陷；锁冲突本身随后在同一天追加解决，见下面「LinkedIn 登录态浏览器排队」。
- 全量测试套件复跑通过，`tests/test_linkedin_how_you_fit_sync.py` 新增一条用例覆盖"某条搜索整体报错时汇总通知如实反映"。
- 涉及文件：`routes_search.py`、`tests/test_linkedin_how_you_fit_sync.py`

### LinkedIn 登录态浏览器排队（2026-09-08）
- 起因：上一条修复只是让"两个同步按钮撞车"这件事**看得见**（通知会说清楚是谁占用的），用户接着问"如果同时点，你要怎样处理"，明确要求排队方案——占用中就告诉用户"XX 正在占用"，占用方跑完后排队的那个自动开始，不需要用户自己重新点。
- 实现：`job_state.py` 新增一个进程内共享的 FIFO 队列原语 `acquire_linkedin_browser(label, start_fn)`/`release_linkedin_browser()`——tracker 同步（收藏/已投递/面试）、How You Fit 单条/批量同步这四类操作在真正起后台线程跑之前先排一次队；抢到就照常起线程，抢不到就把自己的启动函数存进队列，POST 响应里带 `queued: true, queued_behind: "同步 LinkedIn 收藏列表"` 这样的占用者标签；占用者跑完调用 `release_linkedin_browser()`，队列非空就自动把使用权交给队首、在新线程里跑起来。各自原有的 `_tracker_sync`/`_how_you_fit_sync`/`_how_you_fit_batch` 状态字典新增 `queued_behind` 字段，GET 轮询接口能区分"正常同步中"和"排队等 XX"。前端三个触发入口（`syncTrackerStage`/`syncHowYouFitRow`/`syncHowYouFitAll`）和「智能抓取」顺带触发路径都接了这套排队，点击时如果撞上占用会弹 toast 明确告知在等谁。
- 刻意不覆盖 Easy Apply（打开浏览器窗口留给用户自己点提交，没有程序能感知到的"结束"时刻，排在后面可能永远等不到）和「贴链接」的浏览器兜底（一次 HTTP 请求内同步等完，排队会让这次请求不知道要挂多久）；也没有覆盖每日定时任务（`scheduler.py` 直接调 `sync_all_enabled_searches()`，跟用户当前请求路径没有共享调用点，接入需要额外的异步改造，这次没做——用户报的是两个手动按钮撞车）。详见 [tech-solution.md](tech-solution.md#关键决策)。
- 新增 `tests/test_linkedin_browser_queue.py`：覆盖 `acquire_linkedin_browser`/`release_linkedin_browser` 的 FIFO 排队+自动交接，以及"同时点「同步收藏」和「同步全部搜索」，后一个排队等前一个用完自动开始"的端到端场景；全量测试套件（31 个套件 + 7 个 JS 语法检查）复跑通过。
- 涉及文件：`job_state.py`、`routes_search.py`、`static/app.js`、`tests/test_linkedin_browser_queue.py`（新增）、`tests/test_linkedin_tracker_sync.py`、`tests/test_linkedin_how_you_fit_sync.py`、`README.md`、`tech-solution.md`

### 品牌配色统一：群青→墨绿（2026-09-08）
- 起因：为商业化探索做了一版宣传落地页（Artifact 草稿，见下面「商业化探索」条目），用户看完落地页配色后要求把产品本身也统一成同一套"墨绿+近黑"视觉语言，不要落地页和产品是两套颜色
- `static/style.css` 只动了三处色板 token 块（`:root`、`:root[data-theme="dark"]`、`@media (prefers-color-scheme: dark)` 里的 `:root:not([data-theme="light"])`）：`--primary`/`--primary-hover`/`--primary-soft`/`--primary-glow` 从群青 `#2B3AF0` 系换成墨绿 `#1D6E4F` 系（深色模式 `#4FBE93` 系）；中性色 `--bg`/`--bg-elevated`/`--bg-subtle`/`--border`/`--text`/`--text-muted`/`--text-faint` 也从冷灰调统一带上一点绿灰底色（浅色 `#EEF1EA` 系、深色 `#10140F` 系），跟落地页的 `--paper`/`--ink`/`--signal` token 是同一套数值。`--success`/`--warning`/`--danger` 三个语义色不动——避免"表示成功"的绿色跟新的品牌主色混在一起分不清
- 改版前色板已经完全 token 化（全文件只有约 5 处硬编码色值，且都是语义专用色如 `#f5a623` 的"匹配度中等"标记，不是主题色），这次只改了两个 `:root` 块和一个 `@media` 块共 24 行，没有触碰任何组件层规则
- 用 Playwright 截图核对过首页浅色/深色模式，按钮、统计卡片、图表柱状填充、chip 选中态全部正确换色，无控制台报错
- 涉及文件：`static/style.css`

### 商业化探索（讨论于 2026-09-08）
- 用户提出想把这个工具商业化给更多人用，进行了一轮完整讨论：目标用户收窄到"愿意自己部署本地工具、投外企/英文JD的技术向求职者（工程师/AI产品经理优先）"这个小圈子先试水，不是一上来面向所有求职者
- **竞品格局**：分四类查了盈利模式（自动投递类 Simplify/LazyApply/JobCopilot、AI匹配类 Jobright、CRM追踪类 Teal/Huntr/Jobscan、面试准备类 Final Round AI/牛客），核心结论——免费版是标配、付费墙统一设在"AI生成次数"上；一次性买断在这个赛道正在被淘汰（LazyApply 买断转订阅的教训）；牛客走 to-B 卖企业初筛、跟本项目"给个人初筛"的方向不同。缩小到"自动抓取+打分决策"这个具体功能后，**唯一形态完全对齐的直接竞品是 Jobright**（Sonara/LoopCV 偏自动投递、Jobscan 系没有抓取层）
- **定价思路**：用户提议"$5 + LLM API 用户自付"（BYOK），讨论后认为方向对但要分阶段：BYOK 天然贴合"本地工具、不托管"这条现有架构，能避免把多用户的 LinkedIn 抓取合规风险集中到自己身上，代价是筛掉大部分非技术求职者——跟"先在小圈子试水"是同一个判断
- **部署路径决定**：现有代码是单进程本地工具（xlsx 无并发保护、Flask 开发服务器无鉴权），要做成多用户托管 SaaS 需要重写数据层+加账号体系+生产级部署，工作量和合规风险都大一截。决定**先按自托管发布验证需求，托管 SaaS 留到验证通过之后**（参照 n8n/PostHog/Supabase 这类 devtool 先自托管再做托管版的路径）
- **获客方式**：明确排除"爬取 LinkedIn 个人资料做冷邮件/加好友触达"——这类行为爬的是他人身份信息用于未经同意的商业推广，踩 LinkedIn ToS 和 CAN-SPAM/GDPR/个保法风险，且冷启动转化率通常很低；改为在目标人群已经聚集的社区（一亩三分地、PMCAFF、知乎、即刻等）发内容触达
- **宣传落地页已产出**：Artifact 草稿（本地文件 `landed-landing.html`，未合并进本项目代码库），内测招募 + mailto 留资，定价写"内测阶段免费，LLM 调用费用自理"（还没最终定价）。落地页配色定的"墨绿+近黑"随后被要求同步进产品本身，见上面「品牌配色统一」条目
- 定价数字、多用户托管的具体方案都还没最终决定，此条目只记录已经讨论过、达成共识的部分
- 同一天内继续迭代落地页首屏文案：信号点从 3 条扩到 4 条（新增「投递后自动生成面试准备材料，含上传笔记语音作答+AI打分」，故意把"不替你编答案、逼你说真实经历"这个态度写进去，跟面试语音练习模块的定位一致）；就着草稿里的"申请内测"CTA 明确问过用户这是不是真决定——**确认是认真考虑对外开放，不是文案演练**。后续要做的工程评估（账号体系、xlsx 单文件在多用户下的并发/隔离、每个用户自己的 API key 怎么配置）还没展开，只是先把方向坐实

### LLM 调用花费展示（2026-09-08）
- 起因：用户要求"每次用 LLM 的 API 时，显示用了多少 token、哪家、多少钱"，并让先设计好跟用户的交互方式、展示在哪里。排查发现「防幻觉三层加固」（2026-08-29）那次已经把埋点和聚合都做完了——`llm_calls` 表、`GET /api/llm/stats`——唯独没有前端展示，数据一直躺在数据库里没人看得到
- 跟用户过 AskUserQuestion 确认后，选定两个互补的展示位置一起做：① **单次操作即时反馈**——AI 分析、职位/题库对话、语音练习打分完成时，在原有的 toast/对话气泡/通知文案里追加"模型 · tokens · 成本"一行；② **累计消费的常驻入口**——顶栏通知铃旁边新增花费小组件，常驻显示当日花费，点开是今日/7天/30天/全部四档的按任务/按模型明细
- `llm.py` 新增 `start_usage_tracking()`/`pop_usage_summary()`/`usage_text()`，照抄 `_current_task` 那条 ContextVar 的路子，按"一次用户操作"累计 `_CallRecord` 已经算好的 tokens/成本，不碰数据库、不改动原有埋点写库逻辑（取舍详见 [tech-solution.md](tech-solution.md)"LLM 用量展示为什么再开一个 ContextVar"）；`web_helpers.py` 新增 `usage_notification_message()`，给材料生成/面试准备/题库起草/语音练习出题/简历体检/偏好档案刷新这类"后台线程 + 通知铃"的异步流程把用量拼进通知文案
- 同步返回的路由（AI 分析、职位对话、题库单题对话、题库助手对话、语音练习打分）在响应 JSON 里带 `llm_usage_text`，前端 toast/对话气泡下面直接展示；某个模型没配单价时成本显示"成本未知"，不悄悄当 0 元算
- 有意跳过、范围内没做：`classify_origin` 批量国籍分类（一次性历史补量工具，非常规单次操作）、`linkedin_how_you_fit.py` 里 agent 兜底调用（调用次数不确定的多步流程）——这两处的花费仍计入顶栏聚合统计，只是不会单独弹出"这次花了多少"
- 全量测试套件（30 个套件 + 7 个 JS 语法检查）复跑通过，`tests/test_llm_logging.py` 新增用量汇总相关用例（累加、混模型、定不了价时成本置 None、失败调用也要记、没开跟踪返回 None 等）
- **上线当天追加①（用户反馈）**：花费小组件下拉列表的行是标签+数值左右并排，19rem 宽的下拉里长值把"deepseek-v4-pro"这类长模型名从连字符处硬拆成两行，看着很乱；改成标签/数值上下堆叠、行间加分隔线。顶栏那颗徽标常年显示"成本未知"（因为用户用的 DeepSeek 两个模型当时都没配单价）看着像坏了，改成算不出成本时退回显示 token 数、完全没调用过时显示 $0，下拉明细里仍保留精确的"成本未知"（那边有空间、需要精确）
- **上线当天追加②（补 DeepSeek 真实定价）**：DeepSeek 两个模型一直没有 `price_in`/`price_out`，是追加①里"成本未知"的根源。查了 [DeepSeek 官方定价页](https://api-docs.deepseek.com/quick_start/pricing/)（价格画在图里，文本抓取工具读不出来，下载图片直接看才拿到数字）后按"非高峰 + cache miss"这一档补上（Pro $0.66/$1.98 每百万，Flash $0.22/$0.66 每百万；DeepSeek 定价比 Anthropic 多两个维度——cache hit/miss、UTC 高峰/非高峰各两档，选这一档理由详见 [tech-solution.md](tech-solution.md)"DeepSeek 定价为什么只存一个数"）。顺带发现并修正了 `deepseek-v4-pro` 的 note 文案"推理模型，最省钱"是没查过真实价格时的想当然——它其实比 Flash 贵 3 倍，改成如实描述；`README.md`「选用哪个模型」那段列的 Claude Sonnet 5 价格（"$3/$15"）也是过期的（实际是 $2/$10，跟 2026-08-29 那次修 `llm.py` 注册表时的坑是同一个，README 当时没跟着改），一并修正
- 全量测试套件复跑通过（`tests/test_llm_logging.py` 里假设 DeepSeek 没配单价的三处断言按新价格改写，改用一个真正不在注册表里的假模型名测"注册表外→成本 None"这条规则）
- **上线次日追加③（用户拿真实 DeepSeek 后台账单核对，发现对不上，2026-09-09）**：用户把顶栏花费小组件的"今日"跟 DeepSeek API 后台自己的用量页对比，两边调用次数（18 vs 6）、token 数（183,315 vs 52,342）、成本（$0.02 vs ¥1.00）全部对不上。排查出两个独立问题，都修了：
  - **"今日"其实是"过去24小时"，不是自然日**：`GET /api/llm/stats?days=1` 一直是 `now - timedelta(days=1)` 的滚动窗口，昨天傍晚以后的调用也会被算进"今日"。DeepSeek 账单后台跟大多数计费面板一样按自然日展示，两边口径不同天然对不上。改成 `days == 1` 时特判成"本地零点到现在"，`days=7/30` 保留滚动窗口不变（"过去7天/30天"本来没有自然日概念，不存在同样的歧义）
  - **历史调用的成本被聚合查询悄悄当 0 算**：追加②补上 DeepSeek 单价之前落库的 787 条流水里有 775 条 `cost_usd` 是 `NULL`；`models.llm_call_totals()`/`llm_call_stats()` 用的是裸 `SUM(cost_usd)`，SQL 的 `SUM` 会跳过 `NULL` 只加非空的那几条，"今日"里混进的这些历史 `NULL` 行没有让总数显示"成本未知"，而是被悄悄当成 0 元，成本显示因此远低于实际（$0.02 vs 真实约 $0.14）。没有改查询逻辑，而是直接把这批历史数据修好：775 条里 321 条本来就存了 `input_tokens`/`output_tokens`（只是当时没定价），用现在注册表里的单价一次性回填 `cost_usd`；剩下 454 条连 token 数都没有（请求本身就失败了，没拿到 usage），没法补，保持 `NULL`，这部分之后仍会被 `SUM` 正确跳过——不是新 bug，是数据确实缺失
  - 回填后当天（自然日）成本汇总（$0.166 左右，还在随新调用增长）已经跟用户看到的 DeepSeek 账单（¥1.00 ≈ $0.14 美元）基本对得上，验证了追加②里选的"非高峰 cache miss"定价档位是合理估算，不是拍脑袋。调用次数/token 数仍有小幅出入（推测是双方"今日"边界的具体时刻、以及 DeepSeek 账单面板本身的刷新延迟造成的），量级已经从"差 3 倍"收敛到"个位数条目的误差"，判断是可以接受的估算噪音，不再继续深挖
  - 涉及文件：`routes_misc.py`（`days == 1` 特判自然日）；`jobs.db` 里的一次性数据回填（脚本未入库，直接在会话里跑的一次性 SQL）
- 涉及文件：`llm.py`、`web_helpers.py`、`routes_jobs.py`、`routes_interview.py`、`routes_job_actions.py`、`routes_resume.py`、`routes_misc.py`、`static/common.js`（花费小组件全套逻辑）、`static/app.js`、`static/job_detail.js`、`static/bank.js`、`static/interview_practice.js`、`static/style.css`、6 个模板（`index.html`/`resume.html`/`job_interview.html`/`interview.html`/`interview_practice.html`/`job_detail.html`）、`tests/test_llm_logging.py`、`README.md`、[tech-solution.md](tech-solution.md)、[architecture.md](architecture.md)

### 手机端开发：SessionStart Hook 自动装依赖（2026-09-08）
用户提出"本地开发的项目，出门在外用手机怎么继续开发"，方案是 Claude Code on the web/手机 app——每次开新 session 是全新云端容器，只 clone 代码，`config.json`/`jobs.db`/`resumes/`/API key 等本地专属数据和密钥都不在里面（本来就该如此，不建议同步真实数据到云端）。这次单独解决"每次新 session 都要手动 `pip install -r requirements.txt` 才能跑测试"的问题：新增 `.claude/hooks/session-start.sh`，只在 `CLAUDE_CODE_REMOTE=true`（云端容器）时执行，建一个 `.venv`（容器系统 Python 由 apt 管理部分包缺 RECORD 文件，直接装到系统环境会报错，改用虚拟环境隔离）并装好 `requirements.txt`，通过 `$CLAUDE_ENV_FILE` 把 `.venv/bin` 加进后续所有 bash 命令的 `PATH`。在 `.claude/settings.json` 里注册为 `SessionStart` hook。最初做成同步模式（保证依赖装完了 session 才真正开始，避免用测试/lint 时依赖还没装好的竞态），用户体验一次后觉得每次开 session 要多等这一两分钟太慢，改成 **async 模式**（脚本首行输出 `{"async": true, "asyncTimeout": 300000}`，session 立即开始，装依赖转后台跑）——代价是刚进 session 头几十秒如果立刻要跑测试/lint，可能因为依赖还没装完而失败，等一会儿重试即可，这是用户知情后接受的取舍。验证过 `tests/test_llm.py` 单条用例和 `node --check` 都能在（后台）装完后正常执行，hook 重复跑两次幂等无报错。这个 hook 要合并进默认分支之后，以后所有手机端 session 才会自动生效。涉及文件：`.claude/hooks/session-start.sh`（新增）、`.claude/settings.json`。
